...
const computeAutoValue = (formula, form, target, precision = 2) => {
...
const SalesCrudPage = ({ title, endpoint, fields, displayFields, requiredField, searchFields, autoCalc }) => {
  ...
  const handleChange = (e) => {
    ...
    if (autoCalc?.target && autoCalc?.formula) {
      const autoValue = computeAutoValue(autoCalc.formula, updated, autoCalc.target, autoCalc.precision || 2);
      if (autoValue !== null) updated[autoCalc.target] = autoValue;
    }
    setForm(updated);
  };
