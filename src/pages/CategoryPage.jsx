...
const LANG = {
  en: {
    ...
    namePlaceholder: "e.g. Garments",
  },
  ur: {
    title: "کیٹیگری مینجمنٹ",
    subtitle: "اپنی مصنوعات کی کیٹیگریز کا انتظام کریں",
    addBtn: "نئی کیٹیگری شامل کریں",
    searchPlaceholder: "کیٹیگری نام سے تلاش کریں…",
    ...
    reportHeader: "کیٹیگریز کی فہرست",
    printedOn: "پرنٹ کی تاریخ",
    errorMsg: "کیٹیگری کا نام ضروری ہے۔",
    deleteConfirm: "کیا آپ واقعی اس کیٹیگری کو حذف کرنا چاہتے ہیں؟",
    ...
    namePlaceholder: "مثلاً Garments",
  },
};
...
const CategoryPage = () => {
  ...
  const handleLangToggle = async () => {
    ...
    const untranslated = records.filter((r) => !urduCache[`cat:${r.id}`]);
    ...
    try {
      const results = await Promise.all(
        untranslated.map(async (r) => ({
          id:   r.id,
          urdu: await translateText(r.category_name),
        }))
      );
      setUrduCache((prev) => {
        const next = { ...prev };
        results.forEach(({ id, urdu }) => { next[`cat:${id}`] = urdu; });
        return next;
      });
    } catch (err) {
  ...
  const getCategoryName = (r) =>
    isUrdu ? (urduCache[`cat:${r.id}`] || r.category_name || "-") : (r.category_name || "-");

  // ── Form ──────────────────────────────────────────────────────────────────
  const openAdd = () => { setForm({ category_name: "" }); setEditingId(null); setShowForm(true); };

  const openEdit = (r) => { setForm({ category_name: r.category_name || "" }); setEditingId(r.id); setShowForm(true); };

  const handleSave = async () => {
    if (!form.category_name.trim()) { showToast("error", t.errorMsg); return; }

    try {
      setSubmitting(true);
      if (editingId) {
        await axios.put(`${API_BASE}/api/categories/${editingId}`, { category_name: form.category_name.trim() });
        setUrduCache((prev) => { const n = { ...prev }; delete n[`cat:${editingId}`]; return n; });
      } else {
        await axios.post(`${API_BASE}/api/categories`, { category_name: form.category_name.trim() });
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setUrduCache((prev) => { const n = { ...prev }; delete n[`cat:${id}`]; return n; });
  ...
  const filtered = records.filter((r) =>
    [r.category_name, urduCache[`cat:${r.id}`] || ""]
  ...
  const generatePrint = (isPdf = false) => {
    ...
    const rows = filtered.map((r, i) => `
      <tr>
        <td>${i + 1}</td>
        <td><strong>${getCategoryName(r)}</strong></td>
      </tr>`).join("");
...
<table>
  <thead><tr><th style="width:50px">#</th><th>${t.categoryName}</th></tr></thead>
            ...
            <table className="w-full text-sm text-slate-600">
              <thead>
                <tr className="bg-sky-50 text-slate-600 text-xs font-bold border-b border-sky-100">
                  ...
                  <th className={`px-5 py-4 ${isUrdu ? "text-right" : "text-left"}`}>{t.categoryName}</th>
                    ...
                    <tr key={r.id} className="hover:bg-sky-50/70 transition">
                      ...
                      <td className={`px-5 py-4 font-bold text-slate-800 ${isUrdu ? "text-right" : ""}`}>
                        <div className={`flex items-center gap-3 ${isUrdu ? "flex-row-reverse" : ""}`}>
                          ...
                          <span className={translating ? "opacity-40" : ""}>{getCategoryName(r)}</span>
                        </div>
                      </td>
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6" dir={dir}>
              <div className="flex items-center gap-3 mb-6 border-b border-sky-100 pb-4">
                <div className="w-11 h-11 rounded-2xl bg-sky-100 flex items-center justify-center">
                  <i className="bi bi-grid-fill text-sky-700 text-lg"></i>
                </div>
                <h2 className="text-xl font-extrabold text-slate-800">{editingId ? t.edit : t.addBtn}</h2>
              </div>

              <div className="mb-6">
                <label className="block text-xs font-semibold text-slate-500 mb-1.5">{t.categoryName} *</label>
                <div className="relative">
                  <i className={`bi bi-grid absolute top-1/2 -translate-y-1/2 text-slate-400 ${isUrdu ? "right-3" : "left-3"}`}></i>
                  <input
                    type="text"
