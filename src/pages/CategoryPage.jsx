...
const LANG = {
  en: {
    ...
    summarySubtitle: "Overview of visible category records",
    ...
    printBtn: "Print",
    pdfBtn: "Download PDF",
    reportHeader: "Category Names List",
    ...
    formSubtitle: "Category name and classification",
    required: "Required",
    records: "Records",
  ...
  ur: {
    ...
    summaryTitle: "کیٹیگری سمری",
    summarySubtitle: "نظر آنے والی کیٹیگری ریکارڈز کا خلاصہ",
    totalCategories: "کل کیٹیگریز",
    ...
    formSubtitle: "کیٹیگری نام اور کلاسیفکیشن",
    required: "ضروری",
    records: "ریکارڈز",
    placeholder: "مثلاً Garments",
  },
};
...
const CategoryPage = () => {
  ...
  const getCategoryName = (r) =>
    isUrdu
      ? urduCache[`cat:${r.id}`] || r.category_name || "-"
      : r.category_name || "-";
  ...
  const openEdit = (r) => {
    setForm({ category_name: r.category_name || "" });
    setEditingId(r.id);
    setShowForm(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`${API_BASE}/api/categories/${editingId}`, payload);
        setUrduCache((prev) => {
          const next = { ...prev };
          delete next[`cat:${editingId}`];
          return next;
        });
      } else {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="bg-slate-50 rounded-lg border border-slate-200 p-4">
                  <div className="w-10 h-10 rounded-lg bg-white text-indigo-600 flex items-center justify-center shadow-sm mb-3">
                    <i className="bi bi-grids-fill"></i>
                  </div>
                  <p className="text-xs text-slate-500 mb-1">{t.totalCategories}</p>
                  <p className="text-3xl font-extrabold text-slate-950">{summary.totalCategories}</p>
                </div>
                ...
                <div className="bg-slate-50 rounded-lg border border-slate-200 p-4">
                  ...
                  <p className="text-xs text-slate-500 mb-1">{t.categoryName}</p>
            ...
            <table className="same-dark-table w-full text-sm text-slate-600">
              <thead>
                <tr>
                  ...
                  <th className={`${isUrdu ? "text-right" : "text-left"}`}>{t.categoryName}</th>
              ...
              >
                <div className={`flex items-center gap-3 ${isUrdu ? "flex-row-reverse" : ""}`}>
                  <div className="w-11 h-11 rounded-lg bg-indigo-50 flex items-center justify-center">
                    <i className="bi bi-grids-fill text-indigo-700 text-lg"></i>
                  </div>
                  <div>
                  ...
                  <div className="p-4 grid grid-cols-1 gap-3">
                    <div>
                      ...
                      <div className="relative">
                        <i
                          className={`bi bi-grid absolute top-1/2 -translate-y-1/2 text-slate-400 ${
