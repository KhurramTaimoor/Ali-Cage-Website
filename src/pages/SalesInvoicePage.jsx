...
export default function SalesInvoicePage() {
  ...
  const calcRow = useCallback(
    (row) => {
      ...
      if (saleUnit === "carton") {
        if (row.sale_type === "carton") {
          qty = toNum(row.carton_qty);
          amount = qty * pcsCtn * rate;
        } else {
          qty = toNum(row.pieces_qty);
          amount = qty * rate;
        }
      } else {
        qty = toNum(row.qty);
      ...
      prev.map((row, i) => {
        ...
        if (field === "product_id") {
          ...
          if (prod) {
            ...
            next.sale_type = saleUnit === "carton" ? "pieces" : "single";
            next.carton_qty = "";
            next.pieces_qty = "";
            next.qty = "";
          }
  ...
  const preparePayload = () => {
    ...
    const validItems = items
      .filter((row) => row.category_id && row.product_id && row.unit_id && toNum(row.amount) > 0)
      .map((row, idx) => ({
        sr: idx + 1,
        ...
        carton_qty: toNum(row.carton_qty),
        pieces_qty: toNum(row.pieces_qty),
        qty: toNum(row.qty),
        pieces_per_carton: toNum(row.pieces_per_carton),
        rate: toNum(row.rate),
    ...
    <div dir={dir} className="invoice-page">
      ...
      <style>{`
        *{box-sizing:border-box}
        .invoice-page{min-height:100vh;background:linear-gradient(135deg,#f8fafc,#eef2ff);padding:18px;color:#0f172a;font-family:${isUrdu ? "'Noto Nastaliq Urdu', serif" : "Arial, sans-serif"}}
        ...
        .inputModalBox{width:min(1260px,calc(100vw - 24px))!important}.formTopLine{grid-template-columns:150px 250px 140px 140px 280px 1fr!important}.date-box{display:grid;grid-template-columns:118px 1fr!important;gap:6px}.date-preview{min-width:150px;text-overflow:clip!important;white-space:nowrap}.paymentPanel{overflow-x:auto}.basicProductTable{min-width:960px!important;table-layout:fixed}.basicProductTable th:nth-child(1),.basicProductTable td:nth-child(1){width:34px}.basicProductTable th:nth-child(2),.basicProductTable td:nth-child(2){width:115px}.basicProductTable th:nth-child(3),.basicProductTable td:nth-child(3){width:125px}.basicProductTable th:nth-child(4),.basicProductTable td:nth-child(4){width:80px}.basicProductTable th:nth-child(5),.basicProductTable td:nth-child(5){width:90px}.basicProductTable th:nth-child(6),.basicProductTable td:nth-child(6){width:90px}.basicProductTable th:nth-child(7),.basicProductTable td:nth-child(7){width:62px}.basicProductTable th:nth-child(8),.basicProductTable td:nth-child(8){width:70px}.basicProductTable th:nth-child(9),.basicProductTable td:nth-child(9){width:70px}.basicProductTable th:nth-child(10),.basicProductTable td:nth-child(10){width:62px}.basicProductTable th:nth-child(11),.basicProductTable td:nth-child(11){width:70px}.basicProductTable th:nth-child(12),.basicProductTable td:nth-child(12){width:80px}.basicProductTable th:nth-child(13),.basicProductTable td:nth-child(13){width:34px}.productInput{min-width:0!important;padding:5px 6px!important}.finalTotalBar{grid-template-columns:repeat(3,1fr)!important}
        @media(max-width:1250px){.formTopLine{grid-template-columns:repeat(3,minmax(0,1fr))!important}.date-box{grid-template-columns:125px 1fr!important}.inputModalBox{width:calc(100vw - 18px)!important}.finalTotalBar{grid-template-columns:repeat(3,1fr)!important}}
        @media(max-width:780px){.formTopLine{grid-template-columns:1fr!important}.date-box{grid-template-columns:1fr!important}.date-preview{min-width:0}.finalTotalBar{grid-template-columns:1fr!important}.basicProductTable{min-width:900px!important}.inputModalBody{padding:10px!important}}
                ...
                <tr key={inv.id || idx}>
                  ...
                  <td style={{ textAlign: "center" }}>
                    <div style={{ display: "flex", justifyContent: "center", gap: 6, flexWrap: "wrap" }}>
                      <button className="btn btn-green" style={{ padding: "7px 10px" }} onClick={() => openEdit(inv.id)}>{t.edit}</button>
                      <button className="btn btn-yellow" style={{ padding: "7px 10px" }} onClick={() => handlePrint(inv.id)}>{t.print}</button>
                      <button className="btn btn-red" style={{ padding: "7px 10px" }} onClick={() => handleDelete(inv.id)}>{t.delete}</button>
                    </div>
                  </td>
                </tr>
              ))}
                ...
                <table className="basicProductTable">
                  <thead>
                    <tr>
                      ...
                      <th style={{ width: 115 }}>{t.saleType}</th>
                      <th style={{ width: 80 }}>{t.pcsCtn}</th>
                      <th style={{ width: 90 }}>{t.cartonQty}</th>
                      <th style={{ width: 90 }}>{t.piecesQty}</th>
                        ...
                        <tr key={idx}>
                          ...
                          <td>
                            <select className="productInput" value={row.sale_type} onChange={(e) => handleItemChange(idx, "sale_type", e.target.value)} disabled={!isCartonProduct}>
                              ...
                              {isCartonProduct && <option value="pieces">{t.pieces}</option>}
                            </select>
                          </td>
                          ...
                          <td><input type="number" className="productInput" style={{ textAlign: "right" }} disabled={!isCartonProduct || row.sale_type !== "pieces"} value={row.pieces_qty} onChange={(e) => handleItemChange(idx, "pieces_qty", e.target.value)} /></td>
                          <td><input type="number" className="productInput" style={{ textAlign: "right" }} disabled={isCartonProduct} value={row.qty} onChange={(e) => handleItemChange(idx, "qty", e.target.value)} /></td>
