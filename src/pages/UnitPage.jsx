...
const UnitPage = () => {
  ...
  const handleLangToggle = async () => {
    ...
    try {
      ...
      setUrduCache((prev) => {
        const next = { ...prev };
        results.forEach(({ id, urdu }) => { next[`unit:${id}`] = urdu; });
        return next;
      });
    } catch (err) {
  ...
  // ── Form ──────────────────────────────────────────────────────────────────
  const openAdd = () => { setForm({ unit_name: "", symbol: "" }); setEditingId(null); setShowForm(true); };
  ...
  const handleSave = async () => {
    if (!form.unit_name.trim() || !form.symbol.trim()) { showToast("error", t.errorMsg); return; }
    ...
    try {
      ...
      if (editingId) {
        ...
        setUrduCache((prev) => { const n = { ...prev }; delete n[`unit:${editingId}`]; return n; });
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setUrduCache((prev) => { const n = { ...prev }; delete n[`unit:${id}`]; return n; });
  ...
  const generatePrint = (isPdf = false) => {
    ...
    const rows = filtered.map((r, i) => `
      <tr>
        ...
        <td><span style="background:#f1f5f9;padding:4px 8px;border-radius:4px;font-family:monospace">${r.symbol || "-"}</span></td>
      </tr>`).join("");
...
<table>
  <thead><tr>
    ...
    <th>${t.unitName}</th>
    <th style="width:120px">${t.symbol}</th>
  </tr></thead>
  <tbody>${filtered.length ? rows : `<tr><td colspan="3" style="text-align:center">${t.noRecords}</td></tr>`}</tbody>
</table>
            ...
            <table className="w-full text-sm text-slate-600">
              <thead>
                <tr className="bg-sky-50 text-slate-600 text-xs font-bold border-b border-sky-100">
                  ...
                  <th className={`px-5 py-4 ${isUrdu ? "text-right" : "text-left"}`}>{t.unitName}</th>
                  <th className={`px-5 py-4 ${isUrdu ? "text-right" : "text-left"} w-32`}>{t.symbol}</th>
              ...
              <tbody className="divide-y divide-sky-50">
                {loading ? (
                  ...
                  <tr><td colSpan={4} className="px-6 py-12 text-center text-slate-400">{t.noRecords}</td></tr>
                ) : (
                  filtered.map((r, i) => (
                      ...
                      <td className={`px-5 py-4 font-bold text-slate-800 ${isUrdu ? "text-right" : ""}`}>
                        <div className={`flex items-center gap-3 ${isUrdu ? "flex-row-reverse" : ""}`}>
                          <div className="w-8 h-8 rounded-xl bg-sky-100 flex items-center justify-center text-sky-600 flex-shrink-0">
                            <i className="bi bi-rulers text-xs"></i>
                          </div>
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6" dir={dir}>
              <div className="flex items-center gap-3 mb-6 border-b border-sky-100 pb-4">
                <div className="w-11 h-11 rounded-2xl bg-sky-100 flex items-center justify-center">
                  <i className="bi bi-rulers text-sky-700 text-lg"></i>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full border border-sky-100 rounded-2xl py-3 text-sm text-slate-700 bg-sky-50/50 focus:outline-none focus:ring-4 focus:ring-sky-100 font-mono ${isUrdu ? "pr-10 pl-4 text-right" : "pl-10 pr-4"}`}
                    />
                  </div>
                </div>
              </div>
